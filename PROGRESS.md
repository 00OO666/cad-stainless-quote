...
本轮推进**原生轴线对位、平面实例候选定位和逐构件图纸核对表**。使用同名轴线符号的实际圆心而非文字包围框作对应，记录跨图平移及局部残差；区分嵌套的结构与装饰轮廓，不把两层构造当作两个独立制品。定位到一个候选也不等于完成全部报价实例计数。
...
按原生嵌套实体身份审计几何提取异常，局部失败对象的平面投影退化为点，另有近零半径圆弧；保留原始失败日志，不将局部审计推广成全项目完整性证明。现有相关 39 项本地回归通过；本次仅公开脱敏进度文档，**没有新增公开 API、安装变更、完整正确报价行或正式准确率**。固定研发集合和旧冻结结果不改，难例保留并按有界策略转下一构件。
