...
本轮继续做**节点身份、原始边线围面与跨视图尺寸链核验**。复用私有研发工作区已有的闭合面分析 API，比较逐图层与显式跨图层两种模式；记录每条边的来源，保留多个标注落在同一面的情况。某些区域必须由不同图层共同围成，但这样的封闭面可能混合基层、墙体和金属边界，不能直接作为制造展开或实物数量。

连续原生尺寸链可验证不同视图的外包范围与分段是否一致；总长相同不代表内层宽度、加工范围或数量相同。单位未设置的节点仍保留单位未定，跨图同值只生成比例候选，不自动修改整份 CAD 的单位。相关 116 项现有本地回归通过；本次仅公开脱敏进度文档，**没有新增公开 API、安装变更、完整正确报价行或正式准确率**。私有闭合面分析尚未包含在当前公开源码中。
