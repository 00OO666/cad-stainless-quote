...
本次更新新增独立的**等厚直线截面边线配对审计 API**，保留此前视口角度修复。它不是自动下料展开器，也尚未接入 `run`/`resume` 或报价工作簿。下面“开发工作区已验证”的其他内容尚未全部合入公开源码，尤其跨视口尺寸候选阶段仍仅在开发版；不能据本页假设下载版本具有全部研发能力。

## 发布与研发状态

| 层级 | 状态 | 如何理解 |
|---|---|---|
| 公开源码基线 | [ea45ea3](https://github.com/00OO666/cad-stainless-quote/commit/ea45ea3663d93a3544d01b1eef512f3d9ad64fe6) 的预索引功能，加视口角度修复与截面审计 API | 截面 API 需显式指定原生轮廓，未接入自动报价；其他研发能力仍须分别审查 |
...
2. **引线、几何与跨图关系**
   ...
   - 核实同一材料标签可有多个原生引线分支：旧单落点模型会保留歧义。下一步应分别保留分支和目标几何证据，不能通过选第一条、扩大邻近半径或直接拆成多件来消除阻塞。
...
最近一次**开发工作区**回归记录为 **503 项通过、3 项跳过**，涉及变更的 Ruff 检查及环境 doctor 通过。

本轮**公开仓库本地回归**为 **478 项通过、2 项跳过**，全量 Ruff 通过，包括新增截面测试；这是公开工作树的本地结果，不冒充远端 CI 结果。两组测试覆盖不同版本，都不是算量正确率。
...
| P1 | 一材料标签的多引线分支 | 每条原生引线独立保留、映射和绑定目标面片；不把分支数当作实物件数 |
