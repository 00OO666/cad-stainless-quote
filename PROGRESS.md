...
本次修复**已声明的构件身份在取景链路中丢失**：构件取景框、近景记录和图像证据现在保留明确的 `component_id`，使尺寸板能按原构件关联测量候选；不从序号猜身份，也不把身份传递当作实物归属确认。此前编号属性语义修复继续生效。截图成功、候选变多和程序测试通过均不代表报价整行正确。此前公开的多引线和截面 API 尚未接入公开版 `run`/`resume` 自动报价，不能据本页假设下载版本具有全部私有研发能力。
...
最近一次**开发工作区**回归记录为 **586 项通过、3 项跳过**，涉及变更的 Ruff 检查通过；本轮环境 doctor 通过。

本轮**公开仓库本地回归**为 **545 项通过、2 项跳过**，全量 Ruff 通过。已安装版本仅同步本轮两个通用模块的补丁，6 项身份传递回归通过；未覆盖其他私有集成。这是本地结果，不冒充远端 CI 结果。各组测试覆盖不同版本或子集，都不是算量正确率。
