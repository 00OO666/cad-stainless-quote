...
本轮为跨项目开发案例复核与文档更新，**没有发布新源码，也没有新增已完整验证的报价行**。通过原生父块、图框包含关系及尺寸引出点，进一步定位了图号归属与尺寸重叠问题；开发工作簿保留已知人工参照和未确认字段。尚未重跑端到端评分，不据此推算项目完成百分比。
...
最近一次源码变更（2026-09-06）的**开发工作区**回归记录为 **586 项通过、3 项跳过**，涉及变更的 Ruff 检查和当时的环境 doctor 通过。本次为研究产物与进度文档更新，未重跑完整本地回归或端到端报价评测。

上述源码版本的**公开仓库本地回归**为 **545 项通过、2 项跳过**，全量 Ruff 通过。当时已安装版本仅同步两个通用模块的补丁，6 项身份传递回归通过；本次安装版未改。各组测试覆盖不同版本或子集，都不是算量正确率；远端 CI 状态以实际运行记录为准。
