...
本次更新把**原生纸空间尺寸与引线渲染**接入 `component-closeups`：输入明确的构件候选框和原视口后，可以自动生成带原生 DIMENSION 图形的局部图，而非只重画数字文本。截图仍为 REVIEW，不自动确认材料、尺寸角色或数量。此前公开的多引线和截面 API 继续保留；它们尚未接入公开版 `run`/`resume` 自动报价，不能据本页假设下载版本具有全部私有研发能力。
