...
- For CAD-only autonomous research or takeoff before workbook review, add `--no-workbook` and read [references/agent-review.md](references/agent-review.md). Codex performs the semantic review with the existing tools; this flag does not implement an automatic resolver. `--no-render` is independent and does not prevent later targeted original-CAD rendering.
- New runs attach all enhanced node-route candidates to `review-pack.detail_route_context`. Use it only when `available=true`; a missing historical receipt, changed source/index/panel/route, or incompatible version makes it unavailable. Its effective state is REVIEW, not accepted component ownership, dimensions or counts.
...
- When one physical screen combines stainless steel with artistic glass, use the audited `single_line_composite` confirmation in [references/confirmations.md](references/confirmations.md). Emit one quotation row for the complete screen, calculate its whole-elevation projected area, retain both material identities, and never let a stainless-only price match the composite.
...
- `python -m cadquote.view_context_builder` is a standalone, bounded source-replay producer for those contexts. Supply independently reviewed target-free selections and inspect its receipt and negative-search limits. It can trace supported native triangle or circular-divider symbols through endpoint-connected lines to selected native geometry, retaining actual viewport projection and crossing evidence. This does not discover or count physical components automatically. Unsupported, hidden, ambiguous or incomplete paths remain rejected/reviewed, never replaced by same-page proximity. See [references/evaluation.md](references/evaluation.md).
- For unprojected references or embedded OLE warnings in that producer, use its native-page scope evidence and optional `--graphic-reviews` contract described in [references/evaluation.md](references/evaluation.md). Keep every original reference and warning. Explicitly review the actual embedded content before supplying a content review; block names, logo-like placement, empty warning lists and matching hashes alone do not prove harmless content.
...
- For native target page/local-view identity or independently evidenced semantic field comparison, read [references/semantic-identity.md](references/semantic-identity.md). These standalone helpers preserve raw claims and old evaluations; they do not automatically resolve components or confer complete-row accuracy.
...
15. A final row screenshot follows physical-component selection. Exploratory sheet/viewport previews may precede it but remain candidates. Frame the component geometry plus its MT leader and governing dimensions; use a variable-aspect crop rather than a fixed square around the leader point.
