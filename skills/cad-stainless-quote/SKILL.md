...
- After writing explicit row/component occurrence selections, run `scripts/run.ps1 selected-evidence <candidate-boards-json> <panel-index-json> <selections-json> --out <evidence-dir>`. Include a reviewed CAD-coordinate `object_bbox` per group when available. This creates row-specific locator/close-up pairs and blocks one occurrence from being assigned to different components. A leader-point-only crop is explicitly recorded as fallback; all images remain `CANDIDATE` until object geometry, dimensions, quantity, and the cross-drawing chain are confirmed.
